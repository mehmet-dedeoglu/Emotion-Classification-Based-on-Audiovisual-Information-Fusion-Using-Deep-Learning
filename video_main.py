...
from MultimediaCNN import AudioClassifier, FrameClassifier, OutputClassifier
...
def train(fps_t, args_t, fps_test, args_test):
    ...
    x_frames = tf.placeholder("float", [None, 128, 128, 16, 1])
    x_audio = tf.placeholder("float", [None, 131072, 1])
    labels = tf.placeholder("int64", [None])

    y_frame = FrameClassifier(x_frames, use_batchnorm=True, train=True)
    ...
    with tf.Session() as sess:
        ...
        for i in range(args_t.sim_iter):
            for ii in range(10):
                ima_t, aud_t, lab_t = sess.run([x_frames_b, x_audio_b, labels_b])

                # Optimization-Training
                opt = sess.run(optimizer, feed_dict={x_audio: aud_t,
                                                     x_frames: ima_t,
                                                     labels: lab_t})
                loss, acc = sess.run([cost, accuracy], feed_dict={x_audio: aud_t,
                                                                  x_frames: ima_t,
                                                                  labels: lab_t})
                print("Epoch " + str(i) + \
                      " Iter " + str(ii) + ", Loss= " + \
            ...
            for iii in range(20):
                ...
                ima_test, aud_test, lab_test = sess.run(
                    [x_frames_test, x_audio_test, labels_test])
                acc_t, loss_v = sess.run([accuracy, cost],
                                         feed_dict={x_audio: aud_test,
                                                    x_frames: ima_test,
                                                    labels: lab_test})
...
'''
                        # Test visualization.
            ima, aud, labell = sess.run([x_frames, x_audio, labels],
                                        feed_dict={x_audio: aud_t,
                                                   x_frames: ima_t,
                                                   labels: lab_t})
            imaTest, audTest, labellTest = sess.run([x_frames, x_audio, labels],
                                                    feed_dict={x_audio: aud_test,
                                                               x_frames: ima_test,
                                                               labels: lab_test})
            #print(namez1[2])
...
if __name__ == '__main__':
    ...
    setattr(test_args, 'data_dir', '../../../../media/glados/New Volume/Linux/'
                              'Asilomar/Data1/test')
    setattr(test_args, 'batch_size', 12)
