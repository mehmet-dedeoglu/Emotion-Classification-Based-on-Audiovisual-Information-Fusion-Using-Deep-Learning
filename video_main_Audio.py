...
def load_test(fpsTest, batch_sizeTest):
    ...
    iterzTest = dataset_zippedTest.batch(batch_sizeTest).prefetch(20*batch_sizeTest)
...
def train(fps_t, args_t, fps_test, args_test):
    ...
    y = AudioClassifier(x_audio, use_batchnorm=True, train=True)
    ...
    with tf.Session() as sess:
        ...
        for i in range(args_t.sim_iter):
            for ii in range(10):
                aud_t, lab_t = sess.run([x_audio_b, labels_b])
            ...
            for iii in range(5):
                ...
                aud_test, lab_test = sess.run(
                    [x_audio_test, labels_test])
